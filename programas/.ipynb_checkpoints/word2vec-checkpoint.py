...
#reducimos la dimension con pca
pca = manifold.TSNE(perplexity=40, n_components=2, init='pca')
X = pca.fit_transform(X)
# %%
dtf_ = pd.DataFrame(X, index=tot_words, columns=["x","y"])
dtf_["input"] = 0
dtf_["input"].iloc[0:1] = 1
# %%
ax = fig.add_subplot(1,1,1)
#%%
ax.scatter(dtf_['x'],dtf_['y'],'.')
plt.plot(dtf_[dtf_['input']==1]['x'],dtf_[dtf_['input']==1]['y'],'.r')
# %%
for label,row in dtf_[['x','y']].iterrows():
    x,y = row   
...
ax.scatter(dtf_[dtf_["input"]==0]['x'],dtf_[dtf_["input"]==0]['y'], dtf_[dtf_["input"]==0]['z'],'b')
ax.scatter(dtf_[dtf_["input"]==1]['x'],dtf_[dtf_["input"]==1]['y'],dtf_[dtf_["input"]==1]['z'], 'r')
